...
export const usePayrollBeforeDates = (payload: PayrollBeforeDatePayload | null) => {
  const { data, error, isLoading } = useSWR(
    payload && payload.uadId && payload.date ? ['payrollBeforeDates', payload.uadId, payload.date] : null,
    async ([, uadId, date]) => {
...
export const useOvertimeAlerts = (payload: OvertimePayload | null) => {
  const { data, error, isLoading } = useSWR(
    payload && payload.uadId && payload.idPayroll ? ['overtimeAlerts', payload.uadId, payload.idPayroll] : null,
