...
export const PayrollBeforeDatesSelector: React.FC<PayrollBeforeDatesSelectorProps> = ({
    ...
    const { payrolls, loading } = usePayrollBeforeDates(uadId && date ? { uadId, date } : null)
    ...
    return (
        ...
        >
            <option value=''>{loading ? 'Cargando nóminas...' : 'Seleccionar nómina...'}</option>
            {payrolls.map(period => (
                <option key={period.id_payroll} value={period.id_payroll}>
                    {period.description}: {period.date_from.split('T')[0]} to{' '}
                    {period.date_to.split('T')[0]}
                </option>
            ))}
        </select>
    )
